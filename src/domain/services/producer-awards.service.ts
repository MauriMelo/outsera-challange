...
export default class ProducerAwardsService {
  public getMinAndMaxAwardInterval(movies: Movie[]) {
    ...
    for (const movie of movies) {
      for (const producer of movie.producers) {
        ...
        producerMap.interval = producerMap.followingWin - producerMap.previousWin
  ...
  private setMinInterval(min: ProducerAwardInterval[], producerInterval: ProducerAwardInterval) {
    if (!min.length) {
    ...
    } else if (producer !== producerInterval.producer && producerInterval.interval === interval) {
      min.push(producerInterval.clone())
    }
  ...
  private setMaxInterval(max: ProducerAwardInterval[], producerInterval: ProducerAwardInterval) {
    if (!max.length) {
